...
,overview: [
This is the gate-keeping consolidation lesson before solo navigation. It addresses *all* elements. The student plans and flies the entire exercise as pilot-in-command; the instructor is a *silent observer* and intervenes only for safety. All eight elements must be demonstrated and recorded at *S (solo standard)* before the student can be authorised for solo navigation. The route or routes must therefore exercise every element, including a low-level segment and a lost-procedure check. The solo authorisation decision itself is recorded separately (Solo Navigation Authorisation gate / the Solo Navigation lesson checklist). This lesson is repeatable, like Lesson 4, until the standard is achieved, and mandatory Session Notes record the per-element assessment.
]
...
All assessed to solo standard, unassisted.
...
- *1.4* Navigate at low level: identify LSALT and hazards, dead-reckon by ground-based features, configure the aircraft.
- *1.5* Perform the lost procedure: fix position, determine a diversion point within fuel and daylight limits, calculate a revised ETA.
- *1.6* Plan and fly a diversion to a nominated alternative.
- *1.7* Conduct arrival procedures and a landing at a foreign aerodrome; cancel SARWATCH.
- *1.8* Operate the EFB in operational and limited modes, reverting to DR.
...
- Operate the EFB in operational mode, then in limited mode on command, reverting to DR (element 1.8).
- Manage SARWATCH; cancel on completion (elements 1.2, 1.7).
- Fly a low-level segment to solo standard, identifying hazards and navigating by ground-based features (element 1.4).
- Perform the lost procedure to re-establish position when set up by the instructor (element 1.5).
]

,common_errors:[
- *Waiting to be prompted*: At this stage the student leads; hesitation that needs prompting is below solo standard.
- *Diversion not to solo standard*: The diversion must be planned and flown unprompted, with a CLEAROFF and (if diverting to an aerodrome) preparation for arrival.
- *EFB reliance*: When limited mode is called, revert immediately and confidently to DR.
- *Plan not complete, logs not maintained in flight*: Incomplete fuel or alternate planning is a planning deficiency, not maintaining the log in flight is an in-flight monitoring deficiency. Both are not solo standard.
]

,post_flight:[
- *Student Self-Assessment*: Student leads the debrief — what met solo standard and what did not.
- *Instructor Feedback* (10–15 min):
  - Assess and record each element (1.1–1.8) — see standards below.
  - Confirm which elements are at S, which remain P (progressing), and which need further training (NC).
...
- *Next Steps*: If all elements are at S, proceed to the Solo Navigation Authorisation gate. Otherwise, repeat this lesson targeting the deficient elements.
]

,standards_for_progression:[
#text(size:12pt)[*Standard required before solo authorisation*]
...
- *1.4 Low level navigation* — *S* (low-level segment flown safely; hazards identified; navigate by ground-based features).
- *1.5 Perform lost procedure* — *S* (position re-established; diversion point within fuel and daylight limits; revised ETA).
...
- This lesson is repeatable until all elements (1.1 to 1.8) are recorded at S.
- The solo authorisation decision is recorded separately at the Solo Navigation Authorisation gate.
]

,safety:[
- *Weather*: Ceiling/visibility comfortably above VFR minima; conditions suited to a full-length exercise with a diversion.
- *Altitude*: Suitable VFR cruise level clear of CTA steps and SUA; minimum 1,500 ft AGL en route, includes a low level segment not below 500 AGL; avoid overflying houses and do not overfly townships.
...
- *TEM*:
  ...
  - Maintain a buffer from CTA/SUA across the route.
  - Monitor traffic at all aerodromes used.
- *Suggested Routes*: YHEC-GON-DBO-YWSG (TGL)- divert YCAB -GON-YHEC | YHEC-CPMN-BBI-YCDR (TGL)- divert GON -YHEC. YHEC-YMUR-YCAS (TGL)-YSRA-YHEC
